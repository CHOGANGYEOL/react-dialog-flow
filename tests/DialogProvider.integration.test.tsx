...
function ResultDialog() {
  ...
  return <Dialog motionDuration={180}>
...
function OpenAsyncFlow({ onResult }: { onResult: (value: boolean) => void }) {
  ...
  return <button onClick={async () => onResult(await openAsync<boolean>(ResultDialog, { onDismiss: () => false }))}>Open async</button>;
}

function OpenResultFlow({ onResult }: { onResult: (value: unknown) => void }) {
  const { openAsyncResult } = useDialog();
  return <button onClick={async () => onResult(await openAsyncResult<boolean>(ResultDialog))}>Open result</button>;
}

function OpenAndCloseTopFlow({ onDismiss }: { onDismiss: (reason: string) => void }) {
  const { closeTop, openAsync } = useDialog();
  return <>
    <button onClick={async () => { await openAsync<boolean>(ResultDialog, { onDismiss: (reason) => { onDismiss(reason); return false; } }); }}>Open async</button>
    <button onClick={() => closeTop()}>Close top</button>
  </>;
}

function OpenAndCloseAllFlow({ onDismiss }: { onDismiss: (reason: string) => void }) {
...
describe('DialogProvider integration', () => {
  it('resolves openAsync with a completed value after the exit transition', async () => {
    ...
    render(<DialogProvider><OpenAsyncFlow onResult={onResult} /></DialogProvider>);
  ...
  it('returns a dismiss reason for Escape through openAsyncResult', async () => {
    const user = userEvent.setup();
    const onResult = vi.fn();
    render(<DialogProvider><OpenResultFlow onResult={onResult} /></DialogProvider>);
    ...
    await waitFor(() => expect(document.querySelector('dialog')?.dataset.state).toBe('closing'));
    finishDialogExit();
    await waitFor(() => expect(onResult).toHaveBeenCalledWith({ status: 'dismissed', reason: 'esc' }));
  });

  it('uses the supplied dismiss fallback when closeTop closes an async entry', async () => {
    const user = userEvent.setup();
    const onDismiss = vi.fn(() => false);
    render(<DialogProvider><OpenAndCloseTopFlow onDismiss={onDismiss} /></DialogProvider>);
  ...
  it('uses the supplied dismiss fallback when closeAll closes an async entry', async () => {
    const user = userEvent.setup();
    const onDismiss = vi.fn(() => false);
    render(<DialogProvider><OpenAndCloseAllFlow onDismiss={onDismiss} /></DialogProvider>);
  ...
  it('connects Dialog.Title and Dialog.Description to the native dialog', async () => {
    ...
    await userEvent.setup().click(screen.getByRole('button', { name: 'Open result' }));
